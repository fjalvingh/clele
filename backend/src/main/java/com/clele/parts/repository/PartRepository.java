...
public interface PartRepository extends JpaRepository<Part, Long> {
    ...
    /**
     * Search parts by an optional free-text term and/or category. The term matches the part number
     * as a case-insensitive substring and the description via PostgreSQL full-text search (websearch
     * syntax). The category filter matches the given category <em>and all of its descendants at any
     * depth</em> (resolved via a recursive walk of category.parent_id), so picking a higher-level
     * node returns parts in any of its sub-categories. When {@code term} is null only the category
     * filter applies.
     */
    @Query(value = """
